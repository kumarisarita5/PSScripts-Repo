...
param(
    ...
    [string] $VaultResourceGroupName = "Contoso-App-RG",
    [string] $VaultName ="Contoso-App-RSVault",
    [string] $Region ="canadacentral",
    [string] $PolicyName = '24-hour-replication-policy',
    [string] $SourceVmARMIdsCSV = "/subscriptions/1f847e97-a9e0-4335-9e1a-531c116d57e0/resourceGroups/Contoso-App-RG/providers/Microsoft.Compute/virtualMachines/Contoso-AppVM21,/subscriptions/1f847e97-a9e0-4335-9e1a-531c116d57e0/resourceGroups/Contoso-App-RG/providers/Microsoft.Compute/virtualMachines/Contoso-AppVM22",
    [string] $TargetResourceGroupId = "/subscriptions/1f847e97-a9e0-4335-9e1a-531c116d57e0/resourceGroups/Contoso-App-RG-asr",
    [string] $TargetVirtualNetworkId = "/subscriptions/1f847e97-a9e0-4335-9e1a-531c116d57e0/resourceGroups/Contoso-App-RG/providers/Microsoft.Network/virtualNetworks/vnet-ContosoClaimsApp",
    [string] $SourceAvailabilityZone = "2",
    [string] $TargetAvailabilityZone = "1",
    [string] $PrimaryStagingStorageAccount = "/subscriptions/1f847e97-a9e0-4335-9e1a-531c116d57e0/resourceGroups/Contoso-App-RG/providers/Microsoft.Storage/storageAccounts/zonalstgsacontosoapprg",
...
$reverseMappingName = "mapping-zone$TargetAvailabilityZone-to-zone$SourceAvailabilityZone"
$containerMapping = $null

Write-Output "Looking for mapping with name: $mappingName"
Write-Output "Also checking reverse mapping: $reverseMappingName"

try {
    # First try to get all existing mappings to see what's available
    $allMappings = @()
    try {
        $allMappings = Get-AzRecoveryServicesAsrProtectionContainerMapping -ProtectionContainer $container -ErrorAction SilentlyContinue
        Write-Output "Found $($allMappings.Count) existing mappings in container"
        foreach ($mapping in $allMappings) {
            Write-Output "  - Existing mapping: $($mapping.Name)"
        }
    } catch {
        Write-Output "No existing mappings found or error accessing them: $($_.Exception.Message)"
    }

    # Check if our desired mapping already exists (either direction)
    $containerMapping = $allMappings | Where-Object { $_.Name -eq $mappingName -or $_.Name -eq $reverseMappingName } | Select-Object -First 1
    ...
    if ($null -eq $containerMapping) {
        ...
        try {
            ...
            Write-Output "Mapping creation job initiated: $($mappingJob.Name)"
            ...
            do {
                ...
                try {
                    ...
                    Write-Output "Error checking job status: $($_.Exception.Message)"
                    break
                }
            ...
            if ($mappingJob.State -eq 'Failed') {
                ...
                foreach ($er in $mappingJob.Errors) {
                    foreach ($pe in $er.ProviderErrorDetails) {
                        Write-Output "Provider Error: $($pe.ErrorMessage)"
                    }
                    foreach ($se in $er.ServiceErrorDetails) {
                        Write-Output "Service Error: $($se.Message)"
                    }
                }
                # Don't throw here, try to find existing mapping instead
                Write-Output "Mapping creation failed, trying to find existing compatible mapping..."
            } elseif ($mappingJob.State -eq 'Succeeded' -or $mappingJob.State -eq 'CompletedWithInformation') {
                # Get the created mapping
                try {
                    Start-Sleep -Seconds 10 # Give it a moment to be available
                    ...
                    if ($null -ne $containerMapping) {
                        Write-Output "Successfully created and retrieved protection container mapping: $($containerMapping.Name)"
                    } else {
                        Write-Output "Mapping created but could not retrieve it, trying alternative approach..."
                    }
                } catch {
                    Write-Output "Mapping created but error retrieving it: $($_.Exception.Message)"
                }
            } else {
                Write-Output "Mapping creation job state: $($mappingJob.State) after $waitedTime seconds"
            }
        } catch {
            Write-Output "Error creating mapping job: $($_.Exception.Message)"
        }
    } else {
        Write-Output "Using existing protection container mapping: $($containerMapping.Name)"
    }
    
    # If we still don't have a mapping, try to find any compatible one
    if ($null -eq $containerMapping) {
        Write-Output "Trying to find any existing compatible mapping with our policy..."
        $allMappings = Get-AzRecoveryServicesAsrProtectionContainerMapping -ProtectionContainer $container -ErrorAction SilentlyContinue
        $compatibleMapping = $allMappings | Where-Object { $_.PolicyId -eq $policy.Id } | Select-Object -First 1
        
        if ($null -ne $compatibleMapping) {
            Write-Output "Found existing compatible mapping: $($compatibleMapping.Name)"
            $containerMapping = $compatibleMapping
        } else {
            Write-Output "No compatible protection container mapping found"
            # Try one more approach - create a simple mapping name
            $simpleMappingName = "asr-mapping-$SourceAvailabilityZone-$TargetAvailabilityZone"
            Write-Output "Attempting to create mapping with simple name: $simpleMappingName"
            try {
                $mappingJob = New-AzRecoveryServicesAsrProtectionContainerMapping -Name $simpleMappingName -Policy $policy -PrimaryProtectionContainer $container -RecoveryProtectionContainer $container
                Start-Sleep -Seconds 60
                $containerMapping = Get-AzRecoveryServicesAsrProtectionContainerMapping -Name $simpleMappingName -ProtectionContainer $container -ErrorAction SilentlyContinue
                if ($null -ne $containerMapping) {
                    Write-Output "Successfully created mapping with simple name: $($containerMapping.Name)"
                }
            } catch {
                Write-Output "Failed to create mapping with simple name: $($_.Exception.Message)"
            }
        }
    }

} catch {
    Write-Output "Error in protection container mapping setup: $($_.Exception.Message)"
}

# Final check - if we still don't have a mapping, fail gracefully
if ($null -eq $containerMapping) {
    Write-Output "Unable to create or find protection container mapping"
    Write-Output "This might be due to ASR infrastructure not being fully ready"
    throw "Unable to set up protection container mapping for zone-to-zone replication. Please ensure ASR infrastructure is properly configured."
